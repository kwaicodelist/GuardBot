...
if(!args[0]) return message.channel.send('Bir link ve ad yazmalısın. `!emojiekle **link** **emojisimi**`');
if(!args[0].endsWith('.png')) return message.channel.send('Doğru bir link yazmalısın.');
if(!args[1]) return message.channel.send('Bir emoji adı yazmalısın. `!emojiekle **link** **emojisimi**`');
if(['ç', 'ö', 'ü', 'ş', 'İ', 'I', 'ğ', 'Ç', 'Ö', 'Ü', 'Ş', 'Ğ'].includes(args[1])) return message.channel.send('**Türkçe karakter kullanma!**');
